...
function processJson(fileName) {
    ...
    finalExcel[getParticipantName(fileName)] = [
        ...
        `${file.AudReg.Score} [${2 - parseInt(file.AudReg.AttentionCheckAnswers)}] (${file.AudReg.falseAlarms})`, // I changed 2 to 1 (zh)
        file.AudReg.Score,
...
function makeExcel(finalExcel) {
    ...
    worksheet.cell(1, 2).string('AudReg');
    worksheet.cell(1, 4).string('AudReg');
    worksheet.cell(1, 6).string('AudRegIntDiff (T)');
    worksheet.cell(1, 7).string('AudRegIntDiff (F)');
    ...
    workbook.write("../results/PrimaryResults_audtraining.xlsx");
}
...
makeFinalExcelFromJsons(); // Convert JSON files in 3_json_files folder to a single Excel file (PrimaryResults_audtraining). Do not run this function before having JSON files in 3_json_files folder.
