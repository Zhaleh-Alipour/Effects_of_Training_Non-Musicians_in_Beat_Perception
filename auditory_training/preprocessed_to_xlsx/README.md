...
This folder contains code `main_audtraining.js` for converting preprocessed `.xlsx` files into a single spreadsheet file used for subsequent statistical analysis.


## Input / Output
- **Input:** Preprocessed data files located in `data/2_preprocessed_data`
- **Output:**: JSON files (`data/3_json_files`) and final results of the experiment (`results/PrimaryResults_audtraining.xlsx`)
...
2. `makeFinalExcelFromJsons()`
    Converts JSON fils from the `data/3_json_files` folder into a single Excel file (`PrimaryResults_audtraining.xlsx`). 
